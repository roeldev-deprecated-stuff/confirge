 ...
 *
 * Main confirge functions wich are exposed to the outside world.
 */
'use strict';
 ...
 * @param {array} $extensions [] - Optional alternative file extensions to read.
 * @return {object|boolean} Returns `false` on failure.
 */
Confirge.read = function confirgeRead($file, $extensions)
{
    ...
    else if (_.isString($file) && !_.isEmpty($file))
    {
        ...
        {
            if (_.isArray($extensions) && $extensions.length >= 1)
            {
                $result = Utils.readFileExtensions($file, $extensions);
            }
        }
    }
